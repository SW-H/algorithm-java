package codetree;
// https://www.codetree.ai/missions/2/problems/collision-experiment-without-wall/description
...
public class 벽이없는충돌실험 {
    ...
    /*
    1초에 0.5 이동 -> 좌표를 10배로 늘리면 : 메모리 초과
    전체 좌표를 2배로 늘리는 방식 : 시간 초과
*/

/*
public class 벽이없는충돌실험_1 {
    ...
    static class Solver {
        ...
        public int marbleCnt;
        public int curTime;
        public int lastCrushedTime = -1;

        public int MAX = 4001;

        public List<Marble> marbles = new ArrayList<>();
        public int[][] idx = new int[MAX][MAX];

        public List<Marble> nextMarbles = new ArrayList<>();
        public int[][] nextIdx = new int[MAX][MAX];
        ...
        public void solve() throws IOException {
            ...
            for (int tc = 0; tc < T; tc++) {
                ...
                for (curTime = 1; curTime <= MAX; curTime++) {
                    simulate();
                }

                System.out.println(lastCrushedTime);
            }
        }
        ...
        public void simulate() {
            ...
            marbles = (ArrayList<Marble>) ((ArrayList<Marble>) nextMarbles).clone();

            for (int i = 0; i < (int) nextMarbles.size(); i++) {
                int r = nextMarbles.get(i).r;
                int c = nextMarbles.get(i).c;
        ...
        public void moveAll() {
            for (Marble marble : marbles) {
                Pair nextLoc = getNextLoc(marble);

                if (!isInRange(nextLoc)) {
                    return;
                }

                int duplicatedMarbleIdx = nextIdx[nextLoc.r][nextLoc.c];
                //주의!!!!!!!!!
                marble.r = nextLoc.r;
                marble.c = nextLoc.c;
                ////////
                ...
                if (duplicatedMarbleIdx != -1) {
                    Marble remained = collide(marble, nextMarbles.get(duplicatedMarbleIdx));
                    lastCrushedTime = curTime;
//                    System.out.println(curTime + "\n");
                    nextMarbles.set(duplicatedMarbleIdx, remained);
//                    continue;
                } else {

                    nextMarbles.add(marble);
                    nextIdx[nextLoc.r][nextLoc.c] = nextMarbles.size() - 1;
                }
            }
        }

        private Marble collide(Marble marble1, Marble marble2) {
            //영향력이 크다 = 무게가 가장 크거나, 무게가 같은 구슬이 여러 개일 경우 구슬의 번호가 가장 클 경우를 의미합니다.

            if (marble1.weight != marble2.weight) {
                if (marble1.weight < marble2.weight) {
                    return marble2;
                }
                return marble1;
        ...
        private Pair getNextLoc(Marble marble) {
            int nr = marble.r + dr[marble.direction];
            int nc = marble.c + dc[marble.direction];

            return new Pair(nr, nc);
        }

        boolean isInRange(Pair loc) {
            int r = loc.r;
            int c = loc.c;
...
//                Arrays.fill(idx[r], -1);
        ...
        public void input() throws IOException {
            ...
            for (int i = 0; i < marbleCnt; i++) {
                ...
                // (x, y, w, d)
                int r = (Integer.parseInt(stringTokenizer.nextToken()) + 1000) * 2;
                int c = (Integer.parseInt(stringTokenizer.nextToken()) + 1000) * 2;
                int w = Integer.parseInt(stringTokenizer.nextToken());
                String d = stringTokenizer.nextToken();

                Marble marble = new Marble(i + 1, r, c, w, dirToInt(d));

                marbles.add(marble);
                idx[r][c] = marbles.indexOf(marble);
            }
        }

        public int dirToInt(String dir) {
            int[] directions = new int['U' + 1];
            directions['U'] = 0;
            directions['L'] = 1;
            directions['D'] = 2;
            directions['R'] = 3;
 ...
 */
    ...
    /*static class Solver {
        ...
        class Marble implements Comparable<Marble> {
            ...
            int weight;
            int direction;
            public Marble(int num, int weight, int direction) {
                this.num = num;
                this.weight = weight;
                this.direction = direction;
            }
            ...
            public int compareTo(Marble marble) {
                //무게가 가장 크거나, 무게가 같은 구슬이 여러 개일 경우 구슬의 번호가 가장 클 경우를 의미합니다.
                if (this.weight != marble.weight) {
                    return this.weight - marble.weight;
                }
                return this.num - marble.num;
            }
        }
        public int T, marbleCnt;
        public int MAX = 20001;
        public int curTime;
        public int lastCrushedTime = -1;
        public Deque<Marble>[][] board = new Deque[MAX + 1][MAX + 1];
        //ULDR
        public int[] dr = {-1, 0, 1, 0};
        public int[] dc = {0, -1, 0, 1};
        public void solve() throws IOException {
            ...
            for (int tc = 0; tc < T; tc++) {
                ...
                input();
                lastCrushedTime = -1;
                for (curTime = 0; curTime - lastCrushedTime <= MAX; curTime++) {
        ...
        public Deque<Marble>[][] nextBoard;
        public boolean move() {
            nextBoard = new Deque[MAX][MAX];
            ...
            for (int r = 0; r < MAX; r++) {
                for (int c = 0; c < MAX; c++) {
                    ...
                    if ((board[r][c] != null) && (board[r][c].isEmpty() == false)) {
                        isMoved = true;
                        for (Iterator<Marble> it = board[r][c].iterator(); it.hasNext(); ) {
                            Marble marble = it.next();
                            for (int sec = 1; sec <= 5; sec++) {
                                int nr = r + 5 * dr[marble.direction];
                                int nc = c + 5 * dc[marble.direction];
                                if (!isInRange(nr, nc)) {
                                    continue;
                                }
                                nextBoard[nr][nc].add(marble);
                                remove();
                                if (nextBoard[nr][nc].remove(marble) != true) {
                                    break;
                                }
                            }
                        }
            ...
            board = new Deque[MAX][MAX];
            board = nextBoard.clone(); // **얕은복사** 주의
            return isMoved;
        }
        ...
        public void remove() {
            for (int r = 0; r < MAX; r++) {
                for (int c = 0; c < MAX; c++) {
                    if ((board[r][c].isEmpty() == false) && (board[r][c].size() > 1)) {
                        lastCrushedTime = curTime;
                        board[r][c] = board[r][c].stream()
                                .sorted()
                                .collect(Collectors.toCollection(LinkedList::new));
                    }
                    while (board[r][c].size() > 1) {
        ...
        public void input() throws IOException {
            ...
            for (int i = 0; i < marbleCnt; i++) {
                ...
                int w = Integer.parseInt(stringTokenizer.nextToken());
                String d = stringTokenizer.nextToken();
                Marble marble = new Marble(i + 1, w, dirToInt(d));
                board[10 * r + 10000][10 * c + 10000] = new LinkedList<>();
                board[10 * r + 10000][10 * c + 10000].offer(marble);
            }
        }
        public int dirToInt(String dir) {
            int[] directions = new int['U' + 1];
